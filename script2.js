...
async function checkUserRole() {
  try {
    ...
    // ✅ CALL UNIFIED ROLE CHECK API
    const response = await fetch(`${API_URL}/api/check-user-role/${empCode}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
    ...
    localStorage.setItem("is_hr", data.is_hr.toString());
    localStorage.setItem("is_partner", data.is_partner.toString());
    localStorage.setItem("is_manager", data.is_manager.toString());
    localStorage.setItem("is_employee", data.is_employee.toString());
    localStorage.setItem("has_approval_permissions", data.has_approval_permissions.toString());
    
    // ✅ SHOW ROLE BADGE IN UI (OPTIONAL)
    displayUserRoleBadge(data);
    
    // ✅ SHOW/HIDE MANAGER BUTTONS BASED ON PERMISSIONS
    toggleManagerButtons(data.has_approval_permissions);
    
    console.log(`👔 User role: ${data.role}`);
    console.log(`🔐 Approval permissions: ${data.has_approval_permissions}`);
    
    return data.has_approval_permissions;
